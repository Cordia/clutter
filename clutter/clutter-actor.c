...
#include "cogl/cogl.h"
#include "cogl/common/cogl-util.h"
...
struct _ClutterActorPrivate
{
  ...
  gboolean        visibility_detect;
  gboolean        allow_redraw;
};
...
static
gboolean clutter_actor_notify_modified_real(ClutterActor          *actor,
                                            ClutterActor          *child)
{
  return TRUE;
}

/* Move up the tree from this actor, notifying them that they have been
 * modified */
static
void clutter_actor_notify_modified(ClutterActor          *actor)
{
  ClutterActor *a;
  ...
  if (CLUTTER_ACTOR_GET_CLASS(actor)->notify_modified &&
      !CLUTTER_ACTOR_GET_CLASS(actor)->notify_modified(actor, 0))
        return;
  /* notify each parent */
  a = clutter_actor_get_parent( actor );
  while (a)
    {
      if (CLUTTER_ACTOR_GET_CLASS(a)->notify_modified &&
          !CLUTTER_ACTOR_GET_CLASS(a)->notify_modified(a, actor))
        return;
					...
					ClutterUnit  *w)
{
  ...
  cogl_util_mtx_transform (mtx, x, y, z, w);
					...
					ClutterVertex *vertex)
{
  ...
  cogl_util_mtx_transform (mtx_p, &point->x, &point->y, &point->z, &w);
					   ...
					   ClutterFixed   w[4])
{
  ...
  cogl_util_mtx_transform (mtx, &_x, &_y, &_z, &_w);
...
clutter_actor_paint (ClutterActor *self)
{
  ...
  if (clutter_actor_is_on_stage(self))
    {
      if (priv->has_clip)
        {
  ...
  else
    {
      ...
      CLUTTER_NOTE (PAINT,
                "Actor '%s' is not visible, position x1: %i, y1: %i x2: %i, y2: %i ",
...
clutter_actor_class_init (ClutterActorClass *klass)
{
  ...
  klass->notify_modified = clutter_actor_notify_modified_real;
}
 ...
 * automatically by modification functions.
 *
 * This differs from clutter_actor_queue_redraw in that if a damage area
 * has been set for the stage, it does not wipe it and instead try and
 * redraw the entire stage.
 */
void
clutter_actor_queue_redraw_damage (ClutterActor *self)
{
  ClutterActor *stage;
  ...
  if ((stage = clutter_actor_get_stage (self)) != NULL)
    clutter_stage_queue_redraw_damage (CLUTTER_STAGE (stage));
}
...
clutter_actor_unparent (ClutterActor *self)
{
  ...
  if (CLUTTER_ACTOR_IS_VISIBLE (old_parent))
    ...
    clutter_actor_notify_modified( old_parent );
...
/**
 * clutter_actor_is_on_stage:
 * @self: a #ClutterActor
 *
 * Works out whether @self is positioned so that it is on the stage
 * as opposed to off the stage. ONLY WORKS WHEN MATRICES SET UP.
 *
 * Since: 0.8.2-maemo
 */
static gboolean clutter_actor_is_on_stage(ClutterActor *self)
 ...
 * @self: a #ClutterActor
 *
 * Sets whether to use visibility detection or not. If FALSE,
 * clutter_actor_is_on_stage will always return TRUE.
 *
 * Since: 0.8.2-maemo
 */
void clutter_actor_set_visibility_detect(ClutterActor *self, gboolean use)
{
  ClutterActorPrivate *priv;

  if (!CLUTTER_IS_ACTOR (self)) return;

  priv = CLUTTER_ACTOR_GET_PRIVATE(self);
  priv->visibility_detect  = use;
}
 ...
 * Since: 0.8.2-maemo
 */
void clutter_actor_set_allow_redraw(ClutterActor *self,
                                    gboolean allow)
{
  ClutterActorPrivate *priv;
