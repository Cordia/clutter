  ...
  for (source_y = src_y,
      ...
      if (!y_iter.intersects)
	{
      ...
      for (source_x = src_x,
	   ...
	   source_x += inter_w )
        {
          gboolean slice_bmp_alloc = FALSE;
	  /* Skip non-intersecting ones */
	  ...
	  slice_bmp.rowstride = bpp * slice_bmp.width;
	  if (slice_bmp.rowstride != source_bmp->rowstride)
	    {
	      /* we have to allocate memory for this as GLES doesn't allow
	       * the setting of rowstride :( */
              slice_bmp.data = (guchar*) g_malloc (slice_bmp.rowstride *
                                                 slice_bmp.height);
              slice_bmp_alloc = TRUE;
	    ...
	    {
	      /* else create a fake bitmap without allocating */
              slice_bmp.data =
                &source_bmp->data[source_bmp->rowstride*source_y +
                                  bpp*source_x];
              slice_bmp_alloc = FALSE;
	    }
          ...
          if (local_x==0 && local_y==0 &&
              inter_w==x_iter.span->size &&
              inter_h==y_iter.span->size)
            {
              GE( glTexImage2D (tex->gl_target, 0, tex->gl_intformat,
                                x_iter.span->size, y_iter.span->size, 0,
                                tex->gl_format, tex->gl_type, slice_bmp.data) );
            }
          else
            {
			      ...
			      int    height)
{
  ...
  if (!cogl_features_available(COGL_FEATURE_TEXTURE_NPOT)) {
        if (!cogl_util_is_power_2(width) || !cogl_util_is_power_2(height))
                return FALSE;
  }
  return TRUE;
}
...
_cogl_texture_slices_create (CoglTexture *tex)
{
  ...
  /* If we can allocate this texture directly, then just do it!! */
  ...
  if ((tex->max_waste <= -1) || force_no_slice)
    {
      CoglTexSliceSpan span;
			      ...
			      CoglPixelFormat  internal_format)
{
  ...
  /* Check for 16 bit only request */
  if (internal_format == COGL_PIXEL_FORMAT_ANY_16)
    internal_format = _cogl_bitmap_get_16bit_format(&tex->bitmap);
...
/* vertex_format should be one of GL_TRIANGLES, GL_TRIANGLE_FAN, etc */
static void
_cogl_texture_drawarrays (CoglHandle         handle,
		         guint              n_vertices,
		         ...
		         gboolean           use_color)
{
  ...
  GE( cogl_wrap_glDrawArrays (vertex_format, 0, n_vertices) );
...
void
cogl_texture_triangles (CoglHandle         handle,
                        guint              n_vertices,
                        CoglTextureVertex *vertices,
                        gboolean           use_color)
{
