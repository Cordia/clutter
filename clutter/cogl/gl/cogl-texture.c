			  ...
			  GLenum          *out_gltype)
{
  ...
  switch (format)
    ...
    case COGL_PIXEL_FORMAT_ABGR_8888:
       ...
       * ordering (even according to GLES spec). These are
       * available in OpenGL 1.2 onwards with the
       * GL_EXT_packed_pixels extension */
    ...
    case COGL_PIXEL_FORMAT_BGR_565:
      ...
      if (support_packed_pixel)
        ...
        {
          gboolean usr_bgr = (format & COGL_BGR_BIT) != 0;
          gltype = GL_UNSIGNED_BYTE;
          if (usr_bgr)
            required_format = COGL_PIXEL_FORMAT_BGR_888;
          else
...
/* This is a fallback, because we're doing this for GLES where we have
 * something that runs at a decent speed! */
void
